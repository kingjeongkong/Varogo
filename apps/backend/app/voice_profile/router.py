...
from app.voice_profile.schemas import ImportManualRequest, VoiceProfileResponse
...
@router.post('/import-manual', status_code=201, response_model=VoiceProfileResponse)
async def import_manual(
  request: Annotated[ImportManualRequest, Body()],
  current_user: CurrentUser = Depends(get_current_user),
  session: AsyncSession = Depends(get_db),
) -> VoiceProfileResponse:
  profile = await service.import_manual(current_user.sub, request, session)
  return VoiceProfileResponse.model_validate(profile)
